...
class pixip:
                ...
                for y in range(sz):
                    for x in range(sz):
                        if i < ln:
                            ...
                            i += 2
                            TX = i / ln
                            print(f'\rDev: X: {x}, Y: {y}, PX: {i}, TTX: {round(TX * 100 ,2)}%' , end='')
                            sys.stdout.flush()
                        else:
                            pixels[x, y] = (0, 0, 0, 0)
    ...
    def denc(self, out_txt: str = "pix-decoded") -> bool:
        ...
        if os.path.exists(self.in_file) is True:
            try:
                ...
                for y in range(h):
                    for x in range(w):
                        ...
                        if (r, g, b, a) != (0, 0, 0, 0):
                            ...
                            tx = y / h
                            print(f'\rX: {x}, Y: {y}, RGBa: ({r}, {g}, {b}, {a}), TTX: {round(tx * 100, 2)}%', end='')
                            sys.stdout.flush()
                # Remove excess zeros
                ...
                with open(out_txt, 'rb') as filetyp:
                    filetyp = fleep.get(filetyp.read(128))
                test = filetyp.extension
                exte = test[0]
                if os.path.exists(out_txt) is True:
                    os.rename(out_txt, f'{out_txt}.' + exte)
                else:
                    raise FileNotFoundError(f'file {out_txt} not found')
                return True
